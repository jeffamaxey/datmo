...
class ProjectCommand(BaseCommand):
    ...
    def init(self, name, description, force):
        ...
        """
        is_new_model = not self.project_controller.model
        if is_new_model:  # Initialize a new project
            ...
            if not name:
                ...
                name = (
                    default_name
                    if force
                    else self.cli_helper.prompt(
                        __("prompt", "cli.project.init.name"), default=default_name
                    )
                )
            if not description:
                description = (
            ...
            try:
                if success := self.project_controller.init(name, description):
                    self.cli_helper.echo(
        ...
        for k, v in self.project_controller.model.to_dictionary().items():
            if k != "config":
                self.cli_helper.echo(f"{str(k)}: {str(v)}")
        # Ask question if the user would like to setup environment
        environment_setup = (
            False
            if force
            else self.cli_helper.prompt_bool(
    ...
    def version(self):
        return self.cli_helper.echo(f"datmo version: {__version__}")
    ...
    def status(self):
        ...
        elif not latest_snapshot_user_generated:
    ...
    def cleanup(self):
        if response := self.cli_helper.prompt_bool(
