...
class TaskController(BaseController):
    ...
    def _open_workspace(self, name, workspace):
        ...
        workspace_url = self.environment_driver.extract_workspace_url(
            name, workspace)
        return webbrowser.open(workspace_url, new=2)
    ...
    def run(self, task_id, snapshot_dict=None, task_dict=None):
        ...
        before_snapshot_dict[
            'message'
        ] = f"autogenerated snapshot created before task {task_obj.id} is run"
        ...
        elif task_dict.get('command', task_obj.command):
        ...
        try:
            ...
            environment_run_options = {
                ...
                "name": f"datmo-task-{self.model.id}-{task_obj.id}",
                "volumes": {},
            ...
            after_snapshot_dict[
                'message'
            ] = f"autogenerated snapshot created after task {task_obj.id} is run"
    ...
    def stop(self, task_id=None, all=False, status="STOPPED"):
        ...
        if task_id:
            try:
                ...
                time.sleep(1)
                task_obj = self.get(task_id)
            task_match_string = f"datmo-task-{self.model.id}-{task_id}"
